...
class GoodsLogic extends RelationModel
{
    ...
    //查找商品所有规格
    public function findgoodspec($allspecstr,$goods_id)
    {
        $keyarr=array_keys($allspecstr);
        if(!strstr($keyarr[0],"_"))
        {
            $specstr=implode(",",$keyarr);
        }
        else
        {

            foreach ($allspecstr as $key => $value) {
               
                    $specarr[]=explode("_",$value['key']);
                
            }
            $count=count($specarr);
            $count2=count($specarr[0]);
            for($i=0;$i<$count;$i++)
            {
                for($j=0;$j<$count2;$j++)
                {
        ...
        $pspecarr=M("spec_item")->where("id in($specstr)")->select();
        $count3=count($pspecarr);
        
        $c=array();
        for($l=0;$l<$count3;$l++)
        {      
            if(!in_array($pspecarr[$l]['spec_id'],$c))
            {
                $lastarr[$pspecarr[$l]['spec_id']]=M("spec")->where("id = ".$pspecarr[$l]['spec_id'])->find();
                
            }
            $img=M("spec_image")->where("spec_image_id= ".$pspecarr[$l]['id']." and goods_id=".$goods_id)->field("src")->find();
            $pspecarr[$l]['src']=$img['src'];

            $lastarr[$pspecarr[$l]['spec_id']]['son'][]=$pspecarr[$l];
            $c[]=$pspecarr[$l]['spec_id'];
        }
        return $lastarr;
    }
