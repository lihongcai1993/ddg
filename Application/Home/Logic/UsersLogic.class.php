...
class UsersLogic extends RelationModel
{
    ...
    public function login($username,$password){
        ...
        $user = M('users')->where("email='$username'")->find();
        if(!$user){
...
//        $type = I('get.type',0);
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if($address_id == 0)
        {
            ...
            if($c >= 5)
                return array('status'=>-1,'msg'=>'最多只能添加5个收货地址','result'=>'');
        }        
        
        //检查手机格式
       
        if($post['country'] == '国家' || $post['province'] == '省份、州' || $post['city'] == '地级市、县')
            return array('status'=>-1,'msg'=>'请选择地区','result'=>'');
        if($post['address']=='')
            return array('status'=>-1,'msg'=>'地址不能为空','result'=>'');
         if(!$post['consignee'])
            return array('status'=>-1,'msg'=>'收货人不能为空','result'=>'');
        if(!$post['mobile'])
            return array('status'=>-1,'msg'=>'手机号码不能为空','result'=>'');
        ...
        {   
            //如果有其他地址又选择成为默认地址   把其他地址改为0
            if($post['is_default']==1)
            {
                $ddd['is_default']=0;
                M("user_address")->where("user_id = {$post['user_id']} and is_default=1")->data($ddd)->save();
            }
        }
