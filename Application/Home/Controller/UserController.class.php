...
class UserController extends BaseController {
           ...
           $upload = new \Think\Upload();// 实例化上传类
            ...
            if(!$info) {// 上传错误提示错误信息
                $this->ajaxReturn($upload->getError());
                 ...
                 $data['head_pic']="/Public/upload/head_pic/".$info['savepath'].$info['savename'];
                $user_id=$_SESSION['user']['user_id'];
                 $row=M("users")->data($data)->where("user_id = $user_id")->save();
                 if($row)
                 {
                    if(file_exists($_SERVER['DOCUMENT_ROOT']."/".$_SESSION['user']['head_pic']))
                    {
                        unlink($_SERVER['DOCUMENT_ROOT']."/".$_SESSION['user']['head_pic']);
                    }
                    $_SESSION['user']['head_pic']=$data['head_pic'];
                    $this->ajaxReturn(1);
                 }
                 else
                 {
    ...
    public function saveinfo()
    {
        ...
        if(!$d->create($_POST,4))
        {
            $this->ajaxReturn($d->getError());
        }
        else
        {
            $userid=$_SESSION['user']['user_id'];
            $d->where("user_id = $userid")->save();
            $this->ajaxReturn(1);
        }
    }
    ...
    function del(){
        ...
        // 
        $data['order_status']=5;
        $del=M('order')->where('order_id='.$id)->save($data);
        if ($del) {
            $this->ajaxReturn(1);
        }else{
    ...
    function fahuo(){
        ...
        // die;
        $data['order_status']=2;
        $datap['confirm_time']=time();
        $fahuo=M('order')->where('order_id='.$id)->save($data);
        if($fahuo){
            $this->ajaxReturn(1);
        }else{
            $this->ajaxReturn('操作失败');
    ...
    function shouhuo(){
        $id=$_POST['id'];
        $data['order_status']=5;
        $shouhuo=M('order')->where('order_id='.$id)->save($data);
        if($shouhuo){
            $this->ajaxReturn(1);
        }else{
            $this->ajaxReturn('操作失败');
    ...
    public function order_list(){
       ...
       if($search_key)
       {
        ...
        $order_list = M('order')->order($order_str)->where($where." and order_status !=5")->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function address_list(){
        ...
        if($_GET['id'])
        {
            $selectadds=M("user_address")->where("address_id = $_GET[id]")->find();
            $this->assign("selectadds",$selectadds);
        }
        $this->display();
    }
    /*
    ...
    public function add_address(){
      if(IS_AJAX){
            $logic = new UsersLogic();
            $data = $logic->add_address($this->user_id,$_POST['old_address_id'],I('post.'));
